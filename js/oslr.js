...
function loading (){
let url = "https://raw.githubusercontent.com/onekin/OpenSLR/master/graphgist/oslr_guide.html";
console.log (url)
...
// https://neo4j.com/docs/api/javascript-driver/4.1/
  ...
  function checkingConnection (err, rsp){
    ...
    if (!_.isUndefined(rsp) && (_.isUndefined(err) || err.length == 0)){
      connectedToNeo4j();
      setConnectionManagementStatus("Connected to " + host +" with the token " + token);
    }else{
      console.log("ERROR:v: ")
      console.log("err:: "+JSON.stringify (err))
      console.log("rsp:: "+JSON.stringify (rsp))
      disconnectedFromNeo4j()
      setConnectionManagementStatus("Unable to connected to " + host +" with the token " + token)
      new Error("Unable to connected to " + host +" with the token " + token)
  }
}
...
function runCypherDELETE (q, qparams){
  // Run a Cypher statement, reading the result in a streaming manner as records arrive:
  console.log('va:: ' + q)
session
  .run(q, qparams)
  .subscribe({
    ...
    onCompleted: dt => {
      ...
      console.log('closed:: ' + JSON.stringify(dt))
    },
    onError: error => {
...
function run2htmlDELETE(cypher) {
// Clear any existing result
try {
        var head = document.getElementById("result-head"),
        body = document.getElementById("result-body"),
...
/*      session = driver.session({database: 'neo4j'})
      ...
      readTxResultPromise
        .then(result => {
          console.log("PromiseResult1:: "+JSON.stringify(result))
          console.log("PromiseResult2:: "+JSON.stringify(result.records))
        })
        .catch(error => {
          console.log("PomiseError:: "+error)
        })
        .then(() => session.close())
      ...
      session
        ...
        .subscribe({
          ...
          onCompleted: function (metadata) {
            console.log('onCompleted:: ' + JSON.stringify(metadata))
              var time = new Date() - t0;
              var tr = document.createElement("tr");
              var th = document.createElement("th");
              th.setAttribute("colspan", "" + fields.length);
          ...
          onError: function (failure) {
            ...
            if (failure.message.indexOf ('Expected exactly one statement per query')>= 0 ){
              ...
              printError (failure.code + ": " + failure.message);
            }
          }
      });
      */
...
/**
   ...
   * @param callback Function to execute after annotation creation
   */
function commitNeo4JDELETE (query, callback) {
  debuggingg(' QUERY>> ' + query, true)
  let url = host + '/db/neo4j/tx/commit'
  ...
  let settings = {
    ...
    data: { "statements" : [ {"includeStats" : true, "statement" :  query}]}
  }
  debuggingg(JSON.stringify(settings, null, 2), true)
  apiCall(settings, callback)
}
